function [uv_eff] = find_uv_eff(data, model, time_doses, time_pasis)
%disp(['Time Doses: ', num2str(time_doses(1))]);
...
for k=1:length(data.ID)
    ...
    %addevent(model, 'time>=1', 'dc_stim=10000');
    %addevent(model, 'time>=4', 'dc_stim=0');
    ...
    for i=1:30
       ...
       % end
       if ~isnan(doses(index,i))
            addevent(model, ['time>' num2str(time_doses(i))], ['uv_dose=' num2str(doses(index,i))]);
